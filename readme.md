> ## 📌 저장소 안내
>
> - **원본 출처**: 이스트소프트 AI 퀀트 4기 과정에서 **강사님이 Public으로 배포하신 학습용 프로젝트**입니다.
> - **사용 권한**: 자유로운 수정·활용 및 개인 프로젝트로의 편입을 **허락받았습니다.**
> - **본 저장소**: 위 원본을 기반으로 수강생 [@dev-dongwon05253](https://github.com/dev-dongwon05253)이 학습·확장한 결과물입니다.
> - **이관 안내 (2026-07-30)**: 기존 계정 접근 불가로 `EST-Bootcamp-Dongwon` 조직으로 이관했으며, **커밋 히스토리는 원본 그대로 보존**했습니다.
> - **배포 워크플로**: `.github/workflows/deploy.yml`은 자동 실행을 차단(`workflow_dispatch` 수동 실행 전용)해 두었습니다. 사용하려면 저장소 Secrets를 먼저 등록한 뒤 push 트리거를 복원하세요.
> - **라이선스·변경 내역**: 원본 저작권과 라이선스 판단, 원본 대비 작업 내용은 [NOTICE.md](NOTICE.md)에 정리했습니다.
> - **보존 자료 (2026-09-15)**: 강사님 최신본에서 빠진 이전 강의 자료는 지우지 않았습니다. 파일째 삭제된 것은 원래 자리(`docs/10.md`, `test/`)에, 내용이 삭제된 문서의 이전 버전은 [`_archive/`](_archive/2026-09-15-before-upstream-sync/README.md)에 있습니다.
>
> 아래부터는 원본 과정 안내 문서입니다.
